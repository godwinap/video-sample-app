...
#import <AMPlayerKit/AMPlayerKit-Swift.h>
...
- (void)playerItemForSource:(NSDictionary *)source withCallback:(void(^)(AVPlayerItem *))handler
{
    ...
    NSString *uri = [source objectForKey:@"uri"];
    NSString *type = [source objectForKey:@"type"];
    NSString *licenseUrl = [source objectForKey:@"licenseUrl"];
    NSString *fpsCertificateUrl = [source objectForKey:@"fpsCertificateUrl"];
    NSString *fpsDrmToken = [source objectForKey:@"fpsDrmToken"];
    ...
    _assetBuilder = [[AMAssetBuilder alloc] initWithAssetCallback:^(AVURLAsset *amAsset) {
        [self playerItemPrepareText:amAsset assetOptions:nil withCallback:handler];
    }];
    
    [_assetBuilder setContentUrl:uri];
    
    if (licenseUrl != nil && licenseUrl.length > 0) {
        [_assetBuilder setLicenseUrl:licenseUrl];
    }
    
    if (fpsCertificateUrl != nil) {
        [_assetBuilder setAssetParam:AMPlayerConstant.AssetParamsKeyFpsCertificateUrl
                             toValue:fpsCertificateUrl];
    }
    
    if (fpsDrmToken != nil) {
        [_assetBuilder setAssetParam:AMPlayerConstant.AssetParamsKeyFpsDrmToken
                             toValue:fpsDrmToken];
    }
